...
def _load_fold_history(
    ...
    for pth_path in all_pths:
        ...
        histories.append({k: ckpt[k] for k in ("train_mse", "val_mse", "train_r2", "val_r2")})
...
def _best_pth_from_history(
    ...
    """
    Pick the checkpoint with the lowest final-epoch val-MSE and print a
    ranked table of all folds.
    """
    ...
    if best_fold_num is not None:
        ...
        # Match against all_pths regardless of root prefix
        for p in all_pths:
            if os.path.basename(p) == candidate:
                return p

    # Fallback: return whichever file appeared at best_idx in the pth list
    # (histories and all_pths are aligned because _load_fold_history skips
    # old-format files, so we map back via fold_number if possible).
    print(f"  WARNING: could not locate expected file '{candidate}', "
...
def _load_model(pth_path: str, sector_ids: torch.Tensor) -> SmallDataDecoderViT:
    """
    Instantiate SmallDataDecoderViT and load weights from a checkpoint.
...
def _build_sector_ids_and_labels():
    ...
    """
    dummy = np.zeros((1, 457, 457), dtype=np.float32)
...
def main():
    parser = argparse.ArgumentParser(
        description="Run interpretability plots from saved .pth checkpoints."
    )
    parser.add_argument(
        "--pth",
        default=None,
        help="Path to a specific model_fold_N.pth.  "
             "Defaults to the fold with the lowest final val-MSE.",
    )
    parser.add_argument(
        "--root",
        default=".",
        help="Directory to search for model_fold_*.pth files "
             "(default: current working directory).",
    )
    parser.add_argument(
        "--out-dir",
        default="interp_outputs",
        help="Directory where plots are saved (default: interp_outputs/).",
    )
    parser.add_argument(
        "--sample-idx",
        type=int,
        default=-1,
        help="Time index into the distance matrix to use as the input sample.  "
             "-1 (default) uses the last available index.",
    )
    parser.add_argument(
        "--no-data",
        action="store_true",
        help="Skip plots that require the distance matrix (gate values and "
             "LayerScale gammas still work without it).",
    )
    parser.add_argument(
        "--layer",
        type=int,
        default=0,
        help="Block index (0-based) for attention-map plots (default: 0).",
    )
    parser.add_argument(
        "--dm-pkl",
        default=None,
        dest="dm_pkl",
        help="Explicit path to the IQDw{w}.pkl distance-matrix file.  "
    ...
    if args.pth:
        ...
        print(f"\n── Found {len(all_pths)} checkpoint(s) on disk:")
        for pth in all_pths:
            print(f"  {pth}")
        ...
        if fold_history:
            ...
            best_pth = all_pths[-1]
            print(
                f"\n  NOTE: no history found in checkpoints (old-format .pth files?).\n"
                f"  Falling back to last checkpoint: {best_pth}"
            )

        print(f"\n  Selected checkpoint: {best_pth}")

    print(f"\n── Loading model from {best_pth} …")
    ...
    if fold_history:
        ...
        print(
            "\n  NOTE: no training history in checkpoints — skipping fold summary."
        )

    # ── Step 4: parameter-only plots (no distance matrix needed) ─────────
    print("\n── Gate values …")
    ...
    # ── Step 5: data-dependent plots ─────────────────────────────────────
    print("\n── Loading distance matrix …")
    try:
        distance_matrix_raw = extract_distance_matrix(pkl_path=args.dm_pkl)
    except FileNotFoundError as exc:
        print(
            f"\n  WARNING: could not load distance matrix ({exc}).\n"
            "  Skipping data-dependent plots.  Use --no-data to suppress this."
        )
    ...
    print(f"  Using sample index {sample_idx} (T={T})")
